...
for (var i = 0; i < ranges.length; i++) {
  ...
  if (p !== '' && p.indexOf('/memfd') !== 0 && p.indexOf('/dev/ashmem') !== 0) continue;
  if (p.indexOf('jit') !== -1) continue;
  if (r.size < 0x40000) continue;
  candidates.push(r);
}
send({tag: 'info', msg: 'candidates=' + candidates.length
      + ' totalMB=' + Math.round(candidates.reduce(function (a, b) { return a + b.size; }, 0) / 1048576)});
...
send({tag: 'done', dumped: count, errors: errors});
"""
